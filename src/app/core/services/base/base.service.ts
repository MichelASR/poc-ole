...
export class BaseService extends BasicService implements BaseCRUDService {
  constructor(resource: string, http: HttpClient, userContext: UserContextService, header: HeaderVO) {
    super(resource, http, userContext, header);
  }
  ...
  all(data?: any, path?: string, baseUrl?: string, noOptions?: boolean, bearer?: string): Observable<any> {
    ...
    return noOptions ? this.getHttp().get<any>(`${this.getUrlContext(path, baseUrl)}`) :
      this.getHttp().get<any>(`${this.getUrlContext(path, baseUrl)}`, options);
  }
  ...
  remove(param: any, baseUrl?: string, path?: string): Observable<any> {
    ...
    baseUrl = baseUrl ? baseUrl : environment.baseUrl;
    if (param.id) {
      return this.getHttp().delete<any>(`${this.getUrlContext(null, baseUrl)}/${param.id}`, options);
    }
    if (param) {
      options.params = new HttpParams();
  ...
  postFile(item: any, path?: string, params?: any, baseUrl?: string): Observable<any> {
